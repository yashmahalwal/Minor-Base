...
import { toMatchStatus, toMatchBody } from "../../generic/utility/jestMatchers";

// TODO: put after delete, setup instances
const maxSteps = 5;
const totalIterations = 5;

const setupInstances = 10;
            ...
            describe(path, () => {
                ...
                pathArr.forEach((nodeEntry) => {
                    ...
                    test(`${nodeName.method}-${nodeName.key}-${nodeName.index}`, async () => {
                        if (!instance) instance = getRandomElement(collection);
                        ...
                        if (
                            !toMatchStatus(status, statusExpected).pass ||
                            !toMatchBody(resBodyActual, {
                                template: resBodyTemplate,
                                resBodyExpected,
                            })
                        ) {
                            flag = true;
                        } else instance = returnedInstance;
                    });
                });
            });
        });
